...
# config = LLMConfig(model_name="gpt-4o", llm_type="OpenAI", api_token=os.getenv("OPENAI_API_KEY"), temperature=1.2)
config = LLMConfig(model_name="meta-llama/Meta-Llama-3.1-70B-Instruct", llm_type="HuggingFace", api_token=os.getenv("HUGGINGFACEHUB_API_TOKEN"), temperature=0.02)
...
prompt_template = ChatPromptTemplate.from_messages(
  [
      ("system", "You are a Conflict of Interest expert with an IQ of 189. Answer any questions "
                 ...
                 "Please do not change the main scenario the user enters."
                 ...
                 "Then include a summarization of the guidance in bullet points a separate line each (make sure there is a line break)"
                 "Format the bullet points in markdown format with a newline character after each point, using the following syntax: • [point 1]\n• [point 2]\n• [point 3]"),
      ("human", "{input}")
  ]
)
